...
void dance_kana_symbols_finished(tap_dance_state_t *state, void *user_data) {
    switch (state->count) {
        ...
        case 2:
            ...
            break;
        case 3:
            tap_code16(LSFT(KC_EQL));  // "～"※ひらがな入力時
            break;
    }
    reset_tap_dance(state);
}
...
void dance_other_Q_symbols_2_finished(tap_dance_state_t *state, void *user_data) {
    switch (state->count) {
        ...
        case 2:   // 2回タップで中点（・）※ひらがな入力時
            tap_code(LSFT(KC_SLSH));
            break;
        case 3:   // 3回タップで縦棒（｜）   
            tap_code(LSFT(KC_INT3));
            break;
        case 4:    // 4回タップで「\」  
            tap_code(KC_INT1);
            break;
...
tap_dance_action_t tap_dance_actions[] = {
    ...
    [TD_MULTI_DIVID]= ACTION_TAP_DANCE_DOUBLE(LSFT(KC_EQL),KC_SLSH),          //  *  /
    [TD_EQUAL_CARET]= ACTION_TAP_DANCE_DOUBLE(LSFT(KC_MINS),KC_EQL),          //  =  ^    (KC_EQL,LSFT(KC_6)), 
